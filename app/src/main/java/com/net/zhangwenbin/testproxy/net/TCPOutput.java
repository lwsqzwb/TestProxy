...
import com.net.zhangwenbin.testproxy.net.Packet.TCPHeader;
    ...
    private void initializeConnection(String ipAndPort, InetAddress destinationAddress, int destinationPort,
        ...
        if (tcpHeader.isSYN()) {
            ...
            TCB tcb = new TCB(ipAndPort, random.nextInt(Short.MAX_VALUE + 1), tcpHeader.sequenceNumber, tcpHeader.sequenceNumber + 1,
                    tcpHeader.acknowledgementNumber, outputChannel, currentPacket,destinationAddress,destinationPort);
            TCB.putTCB(ipAndPort, tcb);

            String connect = ConnectUtil.getConnect(destinationAddress.getHostAddress(),destinationPort+"");

            try {
                byte[] proxy = new byte[4];
                proxy[0] = 10;
                proxy[1] = 92;
                proxy[2] = (byte) 234;
                proxy[3] = 48;
                destinationAddress = InetAddress.getByAddress(proxy);
...
//                    payloadBuffer.get(method);
//                    ByteBuffer httpHead = ByteBuffer.wrap("POST http://www.zwbnew.xin".getBytes());
//                    while (payloadBuffer.hasRemaining()) {
//                        outputChannel.write(httpHead);
//                    }
//                }

                String data = getString(payloadBuffer);
//                Log.i("trans_data","port:" + tcpHeader.destinationPort);
//                data = "POST http://"+ tcb.mAddress.getHostAddress() + data.substring(5,data.length());
//                if (tcpHeader.destinationPort == 80) {
//                    ByteBuffer httpHead = ByteBuffer.wrap(data.getBytes());
//                    while (httpHead.hasRemaining()) {
//                        outputChannel.write(httpHead);
//                    }
//                } else {
//                    while (payloadBuffer.hasRemaining())
...
//                }
    ...
    public static String getString(ByteBuffer buffer)
    {
        ...
        Log.i("trans_data",buffer.limit() - buffer.position() + "");
        try
        {
            ...
            Log.i("trans_data","data:"+ charBuffer.toString());
